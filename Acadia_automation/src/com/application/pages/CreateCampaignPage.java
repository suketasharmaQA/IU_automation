...
//	private WebElement success_campaign_submit;
	...
	public void create_campaign() throws InterruptedException
	{
		...
		for(int objective_num=0; objective_num <=number_of_objectives-1; objective_num++)
		{
			...
			System.out.println("Testing:  "+hm.get("Titles").toString());
			//Select OBJECTIVE
				...
				if (objective_continue_button.isEnabled())
					...
					{
						System.out.println("------------Objective button is NOT enabled OR objective NOT found" + objective_continue_button.isEnabled());
					}
			
			// CAMPAIGN NAME get campaign name and send to input box
				String set_campaign_name = hm.get("Name of the campaign").toString() + DateFunctions.get_current_timestamp();
			...
			// CATEGORY get drop-down value
				...
				category_dropdown_selected_value.selectByVisibleText(hm.get("Select Category").toString());
				System.out.println("------------["+hm.get("Select Category").toString() +"] as category selected");
			
			
			//CLUSTER SELECTION:
			// Sending input to search for cluster:
				cluster_input_box.sendKeys(hm.get("Audience Cluster").toString());
			// getting all result as a text
				String cluster_forced_input = hm.get("Audience Cluster").toString();
			...
			Javascriptlib.scroll_page_js_code(driver, driver.findElement(By.xpath("//span[contains(text(), content)]")));
			
			//SELECT CONTENT
			//Sending input to search for content:
			content_input_box.sendKeys(hm.get("Content").toString());
			// getting all result as a text
			int j =0;
			String content_forced_input = hm.get("Content").toString();
			...
			//SELECT VIDEO
			video_input_box.sendKeys(hm.get("Video").toString());
			
			// getting all result as a text
			int k =0;
			String video_forced_input = hm.get("Video").toString();
			...
			{
				// Add go to Web link:
				go_to_web_input_box.sendKeys(hm.get("GoToWeb").toString());
				System.out.println("------------["+hm.get("GoToWeb").toString() + " ] Web link selected");
				
			}
			else if(Current_Objective_text.equalsIgnoreCase("Get installs of your application through Google Play"))
			{
				packageNameToInstallApp_input_box.sendKeys(hm.get("GoToWeb").toString());
				System.out.println("------------["+hm.get("GoToWeb").toString() + "]  PlayStore link selected");
			}
			else if(Current_Objective_text.equalsIgnoreCase("Ask people to open an existing application on their device"))
			{
				open_app_input_box.sendKeys(hm.get("GoToWeb").toString());
				System.out.println("------------["+hm.get("GoToWeb").toString() + " ] Package is set");
			}
			...
			notifcation_subject.sendKeys(hm.get("Notification Subject").toString());
			System.out.println("------------["+hm.get("Notification Subject").toString() + " ] Notification Subject is selected");
			
			// Add to Notification Text
			notifcation_text.sendKeys(hm.get("Notification Text").toString());
			System.out.println("------------["+hm.get("Notification Text").toString() + "]  Notification Text is selected");
			...
			select_value.selectByValue(hm.get("Goal Type").toString());
			System.out.println("------------["+hm.get("Goal Type").toString() + "]  Goal Type is selected");
			goal_price.sendKeys(hm.get("Goal Price"));
			System.out.println("------------["+hm.get("Goal Price").toString() + " ] Goal Price is selected");
			
			// Set Maximum budget
			max_budget_flag.click();
			max_budget.sendKeys(hm.get("Max budget").toString());
			
			//Set Maximum metric
			max_metric_flag.click();
			max_metric.sendKeys(hm.get("Max metric").toString());
			//Submit the campaign
			try
			{
